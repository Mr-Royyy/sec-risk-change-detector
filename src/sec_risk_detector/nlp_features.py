...
class RiskChangeAnalyzer:
    ...
    def compare_dataframe(self, sections_df: pd.DataFrame) -> pd.DataFrame:
        """Compare each filing with the immediately previous filing by ticker."""

        required_columns = {"ticker", "filing_date", "risk_text", "risk_word_count"}
        ...
        for _, company_df in clean_df.groupby("ticker", sort=False):
            rows = list(company_df.iterrows())
